    ...
    for point in points:
        for i in range(18):
            if point[i][0] != 0.0 and point[i][1] != 0.0:
                cv2.circle(frame, (int(point[i][0] / scale), int(point[i][1] / scale)), 2, (250, 250, 0), 6)
    def draw_line(pt1_index, pt2_index):
        pt = zip(pt1_index, pt2_index)
        for index_1, index_2 in pt:
            if point[index_1][0] == point[index_1][1] == 0.0 or point[index_2][0] == point[index_2][1] == 0.0:
                continue
            cv2.line(frame,(point[index_1][0], point[index_1][1]), (point[index_2][0], point[index_2][1]), (150, 150, 0), 3)
    draw_line([0, 0, 0, 1, 1, 1, 1], [1, 14, 15, 2, 5, 8, 11])
    draw_line([2, 3, 5, 6, 8, 9, 11, 12, 14, 15], [3, 4, 6, 7, 9, 10, 12, 13, 16, 17])
    return frame

def run(num):

    # cap = cv2.VideoCapture('/home/kevin/IFit/OpenPose/test/test.mp4')
    ...
    while op_cfg.PLAY:
        ...
        op_cfg.FRAME = frame
        if res_point1 is None:
            op_cfg.KEYPOINT = None
        else:
